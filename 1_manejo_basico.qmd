...
Para una inspección básica del _pipeline_ tenemos dos opciones principales:
...
Para la monitorización de proyectos con _pipelines_ más complicados se recomienda el uso de `tar_watch()`. Esta función se debe ejecutar en una nueva sesión de RStudio desde el mismo proyecto en que se ejecuta `tar_make()` y abrirá un Shiny Dashboard con información del progreso.

## Inspección de resultados
Una vez completado el _pipeline_ existe la opción de cargar en el entorno global con `tar_load(target_name)` o de leer directamente el resultado con `tar_read(target_name)` (`tar_name` puede estar indistintamente entrecomillado o no).
...
análisis. El script es `ex_report.qmd`, y en él se especifican los targets que
...
`tarchetypes::tar_quarto()`. Ésta no sólo renderiza el informe, sino que detecta las dependencias entre el informe y el resto de targets, como podemos
